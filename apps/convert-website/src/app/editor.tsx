...
import exampleMessageCard from "../data/example-message-card.json" assert { type: "json" }
...
function Editor() {
    ...
    const { object, submit, isLoading, error, stop } = useObject<unknown, APIRequest>({
        api: '/api/use-object',
        schema: IAdaptiveCardSchema
    });

    const [messageCardInput, setMessageCardInput] = useState<string>(JSON.stringify(exampleMessageCard, null, 2));
                                ...
                                <TabsContent value="code" className="h-full">
                                    <div className="rounded-md border bg-muted h-full">
                                        <Textarea
                                            ...
                                            readOnly
                                            defaultValue={object ? JSON.stringify(object, null, 2) : "Please generate the AdaptiveCard first"}
                                        />
                                    </div>
                                </TabsContent>
                            </Tabs>
